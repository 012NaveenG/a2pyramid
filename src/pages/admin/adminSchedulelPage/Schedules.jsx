...
const Schedule = ({ role, userClass, teacherName }) => {
  ...
  if (role === "admin") {
    ...
    filteredSchedule = schedules.flatMap(item =>
      item.schedule.filter(s => s.teacher === teacherName)
    );
  }
  ...
  return (
    <div className="p-4 mt-10 bg-white dark:bg-slate-900 rounded-lg shadow w-full">
      <h1 className="text-2xl font-semibold mb-4 dark:text-white">Schedule</h1>

      {role === "admin" && (
        <div className="mb-4 max-w-xs">
          <Select value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)} className="w-full">
